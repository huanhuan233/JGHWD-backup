...
from .template import extract_word_titles, allowed_file, extract_pdf_titles, build_title_hierarchy, upload_file_from_frontend
import uuid
...
def extract_titles_view(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # 2. 处理 .docx .pdf 文件（自动生成id和name）
            else :
                # 提取标题结构
            
                structure = upload_file_from_frontend(file)
